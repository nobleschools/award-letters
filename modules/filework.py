...
def process_config(settings_file, campus):
    ...
    # For straight reads:
    for key in ['live_backup_folder', 'live_backup_prefix', 'drive_folder',
                'live_archive_folder', 'campus_list', 'live_award_fields',
    ...
    for input_key in ['key_file', 'current_applications', 'current_roster',
                      'strategies', 'targets', 'colleges', 'sattoact',
                      'bump_list']:
...
def read_dfs(config, debug):
    ...
    dfs['app'] = read_apps(config['current_applications'],
                           config['app_fields'])
    dfs['ros'] = read_roster(config['current_roster'],
                             config['roster_fields'])
